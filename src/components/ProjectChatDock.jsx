  ...
  return (
    <div className="fixed bottom-0 right-4 z-40 w-full max-w-md rounded-3xl border border-borderColor-dark/40 bg-background/95 shadow-2xl backdrop-blur-lg">
      <div className="flex items-center justify-between border-b border-borderColor-dark/40 px-4 py-3">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-textSecondary-light dark:text-textSecondary">Project chat</p>
          <p className="font-semibold">{thread.participants.join(' · ')}</p>
        </div>
        <button type="button" className="text-sm text-textSecondary-light dark:text-textSecondary" onClick={() => toggleModal('showChat')}>
          Close
        </button>
      </div>
      <div className="max-h-72 space-y-3 overflow-y-auto px-4 py-4">
        {thread.messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col ${msg.author === 'You' ? 'items-end' : 'items-start'}`}>
            <div
              className={`rounded-2xl px-3 py-2 text-sm ${
                msg.author === 'You'
                  ...
                  : 'bg-borderColor-dark/30 text-textPrimary-light dark:text-textPrimary'
              }`}
            >
            ...
            <span className="mt-1 text-[0.65rem] text-textSecondary-light dark:text-textSecondary">
              {msg.timestamp} · {msg.read ? 'Read' : 'Delivered'}
            </span>
          </div>
        ))}
        <div className="flex items-center gap-2 text-xs text-textSecondary-light dark:text-textSecondary">
      ...
      >
        ...
        <input
          ...
          className="flex-1 rounded-full border border-borderColor-dark/40 bg-black/20 px-3 py-2 text-sm text-textPrimary-light dark:text-textPrimary focus:outline-none focus:ring-2 focus:ring-primary-gold"
        />
        <button
          type="submit"
